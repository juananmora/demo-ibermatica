...
<div class="content">
	...
	<!-- <table id="shop_table">
		...
		<c:forEach var="item" items="${itemList.pageList}" varStatus="loopStatus">
 					...
 					<div class="dataView">
 						<div class="termView">
 							<img class="imgborder card_img_product_sub img_shop_category"  src="${product.image}">
 						</div>
 						<div class="termView">
 						 <div class="dataView_sub">
	 						 <span class="termView_sub">
	 							Nombre:&nbsp;<c:out value="${product.name}" />
	 						</span>
	 						<span class="termView_sub">
	 							Descripci&oacute;n:&nbsp;<c:out value="${product.description}" />
	 						</span>
	 						<span class="termView_sub">
	 							<c:out value="${item.attribute1}" /><c:out
							value="${item.attribute2}" /> <c:out value="${item.attribute3}" />
							<c:out value="${item.attribute4}" /> <c:out
							value="${item.attribute5}" />
	 						</span>
	 						<span class="termView_sub">
	 							Precio:&nbsp;<fmt:formatNumber value="${item.listPrice}"
								pattern="$#,##0.00" />
	 						</span>
	 						<span class="termView_sub button_shop">
	 							<a
									href="<c:url value="/shop/addItemToCart.do"><c:param name="workingItemId" value="${item.itemId}"/></c:url>">
									<span class="button_table">A&ntilde;adir al carro</span>
								</a>
	 						</span>
 						 </div>
 				...
 				<!-- <tr class="${loopStatus.index % 2 == 0 ? 'even' : 'odd'}"><td>-->
